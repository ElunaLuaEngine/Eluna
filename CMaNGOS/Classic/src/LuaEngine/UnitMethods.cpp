...
int LuaUnit::DurabilityRepair(lua_State* L, Unit* unit)
{
    ...
    sEluna.PushUnsigned(L, player->DurabilityRepair(position, cost, discountMod));
    return 1;
}
...
int LuaUnit::DurabilityRepairAll(lua_State* L, Unit* unit)
{
    ...
    sEluna.PushUnsigned(L, player->DurabilityRepairAll(cost, discountMod));
    return 1;
}
...
int LuaUnit::JumpToCoords(lua_State* L, Unit* unit)
{
    ...
    unit->GetMotionMaster()->MovePoint(x, y, z, speedXY, speedZ);
    return 0;
}
...
int LuaUnit::GetHonorPoints(lua_State* L, Unit* unit)
{
    ...
    sEluna.PushUnsigned(L, player->GetStoredHonor());
    return 1;
}
...
int LuaUnit::SetHonorPoints(lua_State* L, Unit* unit)
{
    ...
    float honor = luaL_checkunsigned(L,1);
    uint32 type = luaL_checkunsigned(L, 2);
    uint32 victim = luaL_checkunsigned(L, 3);
    uint32 victimType = luaL_checkunsigned(L, 4);

    player->AddHonorCP(honor,type,victim,victimType);
    return 0;
}
